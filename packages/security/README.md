...
* [Authentication](#authentication)
    ...
    - [Custom Authentication Provider](#custom-authentication-providers)
...
- `secret` - used to encode/decode the token:
    1. String type - hard to guess string
    2. `Rsa` - object with the following properties:
...
[`EncoderFactory`](https://github.com/rxstack/rxstack/blob/master/packages/security/src/password-encoders/encoder-factory.ts) 
...
```typescript
    ...
    // by encoder name
    const encoder: PasswordEncoderInterface = injector.get(EncoderFactory).getByName('some-encoder');
```
...
```typescript
const tokenAuthenticationProvider = injector.get(AuthenticationProviderManager).getByName(TokenAuthenticationProvider.PROVIDER_NAME)
```
...
- [`Token`](https://github.com/rxstack/rxstack/blob/master/packages/security/src/models/token.ts): token class
- [`TokenAuthenticationProvider`](https://github.com/rxstack/rxstack/blob/master/packages/security/src/authentication/token.authentication-provider.ts): authentication provider that supports `Token` class
- [`TokenExtractorListener`](https://github.com/rxstack/rxstack/blob/master/packages/security/src/event-listeners/token-extractor-listener.ts): extracts the raw token and sets `Token` object in the `Request`.
- [`AuthenticationTokenListener`](https://github.com/rxstack/rxstack/blob/master/packages/security/src/event-listeners/authentication-token-listener.ts): authenticates the token
...
Under the hood it uses [`SecurityController`](https://github.com/rxstack/rxstack/blob/master/packages/security/src/controllers/security-controller.ts) which has the following actions:
